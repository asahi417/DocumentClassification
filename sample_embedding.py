...
def get_options(parser):
    ...
    parser.add_argument('-c', '--cut', action='store', nargs='?', const=None, default=1, type=int,
                        choices=None, help='Cut off frequency. (default: 2)', metavar=None)
...
if __name__ == '__main__':
    ...
    if args.data == "sst":
        data = sequence_modeling.sst("./data/stanfordSentimentTreebank", binary=True, cut_off=args.cut)
    ...
    sequence_modeling.vectorize_chunk(sentences=sentences, label=label, length=args.pad, chunk_size=5000,
                                      ...
                                      save_path="./data/embed_p%i_c%i" % (args.pad, args.cut))
