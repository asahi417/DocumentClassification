...
class CharLSTM(object):
    ...
    def _create_network(self):
        ...
        with tf.variable_scope("word_level"):
            ...
            _layer = tf.nn.dropout(self.x_word, self.keep_prob)
        ...
        with tf.variable_scope("character_level"):
            ...
            _layer = tf.nn.dropout(self.x_char, self.keep_prob)
            _layer = embedding_char(_layer, batch_norm=self.batch_norm)
            (output_fw, output_bw), (states_fw, states_bw) = \
        ...
        if self.binary_class:
            _layer = full_connected(_layer, [_shape[-1], 1], batch_norm=self.batch_norm)
            # last layer to get logit and prediction
            _logit = tf.squeeze(_layer)
            ...
            _logit = full_connected(_layer, [_shape[-1], self.network_architecture["label_size"]],
        ...
        # Decay learning rate: lr = lr/lr_decay
