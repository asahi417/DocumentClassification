        ...
        for ind, _d in enumerate(data):  # loop for sentence
            ...
            for __d in _d.split(' '):  # loop for word
                ...
                if __d is not None:
                    ...
                    _oh = padding(np.vstack([np.zeros(26), np.eye(26)])[__vec], self.__length_char)
                    _vec.append(_oh)
            # if no word is contained in sentence, give zero vector with shape (1, word, char)
            _vec = np.zeros((1, self.__length_word, self.__length_char)) if len(_vec) == 0 else np.array(_vec)
